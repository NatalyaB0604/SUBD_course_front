...
import { Checkbox, FormGroup, FormControlLabel, Container, TextField, Button, Typography, Box, InputLabel, FormControl, Select, MenuItem } from '@mui/material';
...
const EditCourse = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        const departmentsResponse = await axios.get('http://localhost:8080/vunder-kids/departments/get-all');
        setDepartments(departmentsResponse.data);

        const courseDepartments = new Set(course.departments.map(department => department.departmentId));
        setSelectedDepartments(courseDepartments);
      } catch (error) {
  ...
  const handleDepartmentChange = (departmentId) => {
    ...
    if (updatedDepartments.has(departmentId)) {
      updatedDepartments.delete(departmentId);
    } else {
      updatedDepartments.add(departmentId);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const departmentsToSend = Array.from(selectedDepartments).map(departmentId => ({
        departmentId: departmentId
    }));

    formData.append('departments', JSON.stringify(departmentsToSend));
        ...
        <FormGroup>
          {departments.map(department => (
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={selectedDepartments.has(department.departmentId)}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              <TextField
                ...
                onChange={(e) => {
                  ...
                  if (/^\d*\.?\d*$/.test(value)) {
                    const updatedPricing = [...pricing];
                    updatedPricing[index].pricePerClass = value;
                    setPricing(updatedPricing);
                  }
                }}
                required
  ...
  return (
    <Container maxWidth="sm" sx={{ p: 3, margin: 'auto' }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <TextField label="Название курса" fullWidth value={courseName} onChange={(e) => setCourseName(e.target.value)} required sx={{ mb: 2 }} />
        <TextField label="Описание курса" fullWidth multiline rows={4} value={courseDescription} onChange={(e) => setCourseDescription(e.target.value)} required sx={{ mb: 2 }} />
        <TextField label="Дата начала" type="date" fullWidth value={startDate} onChange={(e) => setStartDate(e.target.value)} required sx={{ mb: 2 }} InputLabelProps={{ shrink: true }} />
        <TextField label="Дата окончания" type="date" fullWidth value={endDate} onChange={(e) => setEndDate(e.target.value)} required sx={{ mb: 2 }} InputLabelProps={{ shrink: true }} />
        <TextField label="Количество мест" type="number" fullWidth value={availableSpots} onChange={(e) => setAvailableSpots(e.target.value)} required sx={{ mb: 2 }} inputProps={{ min: 0 }} />
        <input type="file" onChange={handleImageChange} accept="image/*" style={{ marginBottom: '16px' }} />

        {/* Условный рендеринг филиалов */}
        {id !== '7' && (
          <>
            <Typography variant="h6" gutterBottom textAlign={'center'}>
              Филиалы
