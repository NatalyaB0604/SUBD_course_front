...
const ParentAccount = () => {
  ...
  const handleUpdateUserInfo = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:8080/vunder-kids/parents/update/${parentId}`, cleanUserData);
      if (response.status === 200) {
        alert('Информация успешно обновлена');
        setIsEditing(false);
      } else {
  ...
  const currentYear = new Date().getFullYear();
  const minBirthDate = `${currentYear - 16}-01-01`;
  const maxBirthDate = currentDate.toISOString().split('T')[0];
    ...
    <Box sx={{ p: 3, maxWidth: '600px', margin: 'auto' }}>
      ...
      {!isEditing ? (
        ...
        <Box sx={{ mt: 3 }}>
          ...
          <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-start', flexDirection: 'column', gap: 1 }}>
            <Button onClick={handleUpdateUserInfo} variant="contained" color="info" sx={{ fontSize: '14px' }} disabled={emailError}> {/* Disable button if email is invalid */}
              Сохранить изменения
            </Button>
            <Button onClick={() => setIsEditing(false)} variant="outlined" color="info" sx={{ fontSize: '14px' }}>
              Отмена
