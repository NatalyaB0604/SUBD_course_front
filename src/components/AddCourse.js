...
import { Container, TextField, Button, Typography, Box, InputLabel, FormControl, Select, MenuItem, FormGroup, FormControlLabel, Checkbox } from '@mui/material';
import { useNavigate } from 'react-router-dom';
import CoursesService from '../services/CoursesService';
import DepartmentsService from '../services/DepartmentsService'; // Предполагается, что у вас есть такой сервис
...
const AddCourse = () => {
  ...
  const [error, setError] = useState('');
  const [availableSpots, setAvailableSpots] = useState(0);
  const [departments, setDepartments] = useState([]); // Состояние для филиалов
  const [selectedDepartments, setSelectedDepartments] = useState(new Set()); // Состояние для выбранных филиалов
  const navigate = useNavigate();

  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        const response = await DepartmentsService.getAllDepartments(); // Получаем все филиалы
  ...
  const handleSubmit = async (e) => {
    ...
    const departmentsToSend = Array.from(selectedDepartments).map(departmentId => ({ departmentId }));
